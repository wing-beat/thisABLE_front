...
function ReviewPage(locationId) {
      ...
      data: [
        {
          ...
          good: 0,
          bad: 0,
          detail: "매장이 넓고 경사로가 있어서 이용하기 편리했어요 카페 내부도 예뻐요! 입구는 하나라 경사로 찾기 쉬울 것 같아요",
          star: 4.5,
  ...
  const renderReviews = reviews && reviews.data.map(review => {
  ...
  return (
    <div>
      <div className='reviewcontainer'>
        ...
        <input className='reviewinput' onChange={(event) => setInputValue(event.target.value)} placeholder='작성한 후기는 익명으로 등록됩니다.' />
        <button className='reviewinputbutton' onClick={postReview}>등록</button>
      </div>
